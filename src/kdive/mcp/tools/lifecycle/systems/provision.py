...
"""The ``shape`` filter value selecting full-custom Systems (``shape IS NULL``, ADR-0067)."""
...
def _viewer_projects(ctx: RequestContext) -> list[str]:
    ...
    ``ctx.projects`` (membership) and ``ctx.roles`` (role grants) are independent — a member
    project can carry no role. ``viewer`` is the lowest role, so any non-``None`` role
    satisfies it; a bare member (no role) is omitted, the same no-leak shape as an ungranted
    project (ADR-0023). The ``is not None`` guard is load-bearing — a rank-only check would
    pass bare members.
    """
...
class _SystemFilters:
    """The validated, SQL-ready clauses + params for a :func:`list_systems` query."""
...
def _build_filters(
    ...
    *,
    ...
    """Translate the filter args into SQL clauses, or a ``configuration_error`` envelope.

    Every filter is a SQL predicate so the row cap applies to the fully-filtered set (no
    post-cap Python filtering that could undercount matches). A malformed ``allocation_id``,
    an unknown ``state``, or a malformed / unsupported ``pcie`` spec is a structured
    ``configuration_error``.
    """
...
def _pcie_clause(pcie: str, params: list[object]) -> Composable | ToolResponse:
    ...
    The ``pcie_claim`` snapshot is the *resolved* vendor:device list (ADR-0068); it carries
    no class code, so a ``class=`` spec cannot be honored against it and is rejected (a
    silent false-empty would mislead an agent into provisioning instead of reusing). A
    matched ``vendor:device`` spec becomes an ``EXISTS`` over the joined allocation's
    ``pcie_claim`` jsonb array, mutating ``params`` in place.
    """
    try:
...
async def list_systems(
    ...
    *,
    ...
    A ``viewer`` read scoped to the caller's projects (ADR-0070): a System in an ungranted
    project — or a member project with no role — is omitted, indistinguishable from absent
    (no-leak, ADR-0023). ``shape`` accepts a named shape or :data:`CUSTOM_SHAPE_SENTINEL`
    for full-custom (``shape IS NULL``); ``pcie`` matches a ``vendor:device`` spec against
    each System's allocation ``pcie_claim``. Mirrors ``allocations.list`` pagination + cap.
    """
...
def _systems_collection(systems: list[System]) -> ToolResponse:
    """Render Systems into one collection envelope (failed → failure item)."""
    return ToolResponse.collection(
        "systems",
        "ok",
        [_envelope_for_system(system) for system in systems],
