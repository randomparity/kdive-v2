"""Shared kernel-build helpers for both libvirt providers (ADR-0096).
...
# catalog fragment. Shared so the validation-time (MCP tool) and execution-time (resolver)
# substitutions cannot diverge. ``provider`` is decorative for build configs (the catalog is
# keyed by name alone) but the ref model requires it; ``system`` matches the seed tenant.
_DEFAULT_CONFIG_REF = CatalogComponentRef(kind="catalog", provider="system", name="kdump")
