...
Within the existing `async with op, admin:` block, after `teardown`:
...
2. Over a fresh `LiveStackClient.over_http(base_url, auditor_token)`, call
   `accounting.report` with `scope="all-projects"`. `_ok(...)` it (non-failure envelope).
...
5. **DB cross-check** (`_ledger_sums(db_url, project)`): sum `kcu_delta` filtered by
   `event_type` from the `ledger` table for `_PROJECT`, quantized via the domain
   `quantize_kcu`. Assert the wire row's `reserved`/`reconciled`/`variance` equal the DB
   `reserved`/`reconciled`/`reconciled − reserved`. This is what proves "real spend."
6. **Emit the artifact** (`_write_report_artifact(payload) -> Path`): write the asserted
   row + the cross-project `total` as `accounting-report.json` under
   `KDIVE_ARTIFACT_DIR` (default `<repo>/.live-stack-artifacts/`, created if absent). Assert
   the file exists and its parsed content round-trips the asserted `reserved`/`reconciled`/
   `variance` for `_PROJECT`.
...
- `async def _ledger_sums(db_url, project) -> tuple[Decimal, Decimal]` — `(reserved,
  reconciled)` quantized DB sums.
- `def _report_artifact_dir() -> Path` — resolve `KDIVE_ARTIFACT_DIR` or the repo default.
...
1. The report reflects the run's real spend (wire rollup `== ` DB ledger sums) and is written
   as a JSON artifact whose content is re-asserted.
...
4. Separate commit from D; D's phases unmodified beyond appending `report`.
