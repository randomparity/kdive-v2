...
- [ ] **Step 1 (tests first):** Write `tests/providers/local_libvirt/test_install.py`. Inject fakes for every slow/host op (`connect` → a fake `_LibvirtConn` recording `defineXML`/`lookupByName`/`create`/`destroy`; `fetch_kernel`/`fetch_initrd` → write canned bytes to the given path; `readiness` → a canned structured result; an injected `boot_window`/`clock` so no real wait):
  - **install render:** `install(system_id, kernel_ref, cmdline="… crashkernel=256M …")` calls `defineXML` once with XML that parses (via `ET.fromstring`) to a `<domain>` whose `<os>` has `<kernel>`, `<initrd>`, and `<cmdline>` text equal to the cmdline, and whose `<kernel>`/`<initrd>` point at the **per-Run staging path** `…/{system_id}/{run_id}/…` (pass `run_id` to `install` or derive the path deterministically — see Step 2);
...
- [ ] **Step 2:** Implement `src/kdive/providers/local_libvirt/install.py`:
  ...
  - `LocalLibvirtInstall(__init__(*, connect, fetch_kernel, fetch_initrd, readiness, staging_root, boot_window))` — slow ops injected; `from_env()` defaults `connect` to `lambda: libvirt.open(KDIVE_LIBVIRT_URI)`, `fetch_*` to object-store reads, `readiness` to the ported preflight, `staging_root` to `KDIVE_INSTALL_STAGING` (default under `/var/lib/kdive`), `boot_window` to a small default.
  - `install(system_id, run_id, kernel_ref, *, cmdline)`: compute the per-Run staging dir `staging_root/{system_id}/{run_id}`; `fetch_kernel`/`fetch_initrd` into it via temp-file-then-rename; render the direct-kernel `<os>` domain XML with `ET` (reuse `domain_name_for`, the kdive metadata tag, the rootfs disk; **add** `<os>` `<kernel>`/`<initrd>`/`<cmdline>`); `defineXML` (libvirt error → `INSTALL_FAILURE`). The kdump-capture-present check is the handler's (Step via runs.py Task 3), not here — `install()` stages + redefines only.
...
- [ ] **Step 0 (read first):** Re-read `runs.py`'s existing `build_run`/`_build_locked`/`_authorizing`/`_run_job_envelope`/`_config_error`/`_stale_handle` — the install tool **reuses** `_authorizing`, `_run_job_envelope`, `_config_error`, `_as_uuid`, the `RUN` lock, and `queue.enqueue` (already imported). Confirm `BuildProfile.parse` is the cmdline source (the Run's `build_profile` may carry an optional `cmdline`; absent → a default that includes `crashkernel=`). Decide the cmdline field: read it from `run.build_profile` as an opaque optional `cmdline` key (the build plane's `BuildProfile` is `extra="forbid"`, so the cmdline is **not** a `BuildProfile` field — read it from the raw `run.build_profile` dict, defaulting when absent). Record this read in the test as the contract.
...
- [ ] **Step 1 (tests first):** Add `install_handler` tests (call the handler directly with a fake `Installer` and a real claimed `Job`):
  ...
  - **kdump prerequisite absent:** the readiness/kdump seam reports the capture path missing → `CategorizedError(CONFIGURATION_ERROR)`, no ledger row;
