...
- [ ] **Step 3 (`live_vm` real-make test):** Add a `live_vm`-gated test that runs the real `make` against `$KDIVE_KERNEL_SRC` and asserts the extracted build-id equals `readelf -n vmlinux`'s build-id (extraction against a real ELF). Gate it with the existing `live_vm` marker so CI skips it. Confirm `-m "not live_vm"` skips it.
...
- [ ] **Step 1 (tests first):** Add `build_handler` tests (call the handler directly with a fake `Builder` + fake store and a real claimed `Job`, mirroring the provision-handler tests):
  ...
  - **store-then-record window:** simulate a crash by recording artifacts then NOT the ledger (call the build seam, skip step 4) and re-dispatch → the deterministic keys are overwritten (assert the put is keyed identically), no orphan row.
