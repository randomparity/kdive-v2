...
def test_project_form_non_member_denied(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            ...
            ctx = _ctx(roles={"proj-a": Role.ADMIN}, projects=("proj-a",))
            resp = await audit_tools.query(pool, ctx, project="proj-b")
        assert resp.status == "error"
        assert resp.error_category == "authorization_denied"
...
def test_filter_by_principal_cross_project(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            await _seed_two_projects(pool)
            ctx = _ctx(platform_roles=frozenset({PlatformRole.PLATFORM_AUDITOR}))
            resp = await audit_tools.query(pool, ctx, principal="alice")
        assert resp.status == "ok"
...
def test_filter_by_object_project_form(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            ...
            ctx = _ctx(roles={"proj-a": Role.ADMIN}, projects=("proj-a",))
            resp = await audit_tools.query(pool, ctx, project="proj-a", object_id=str(ids["a2"]))
        assert resp.status == "ok"
        rows = _rows(resp)
        assert len(rows) == 1
...
def test_filter_by_transition_cross_project(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            await _seed_two_projects(pool)
            ctx = _ctx(platform_roles=frozenset({PlatformRole.PLATFORM_AUDITOR}))
            resp = await audit_tools.query(pool, ctx, transition="defined")
        assert resp.status == "ok"
...
def test_filter_by_window_cross_project(migrated_url: str) -> None:
    ...
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            ...
            resp = await audit_tools.query(pool, ctx, window=window)
        assert resp.status == "ok"
        rows = _rows(resp)
        assert len(rows) == 1
...
def test_malformed_object_id_is_config_error(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            await _seed_two_projects(pool)
            ctx = _ctx(roles={"proj-a": Role.ADMIN}, projects=("proj-a",))
            resp = await audit_tools.query(pool, ctx, project="proj-a", object_id="not-a-uuid")
        assert resp.status == "error"
...
def test_malformed_window_is_config_error(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            ctx = _ctx(platform_roles=frozenset({PlatformRole.PLATFORM_AUDITOR}))
            resp = await audit_tools.query(pool, ctx, window=["not-a-date", None])
        assert resp.status == "error"
        assert resp.error_category == "configuration_error"
...
def test_naive_window_bound_is_config_error(migrated_url: str) -> None:
    async def _run() -> None:
        async with _pool(migrated_url) as pool:
            ctx = _ctx(platform_roles=frozenset({PlatformRole.PLATFORM_AUDITOR}))
            resp = await audit_tools.query(pool, ctx, window=["2026-01-01T00:00:00", None])
        assert resp.status == "error"
        assert resp.error_category == "configuration_error"
